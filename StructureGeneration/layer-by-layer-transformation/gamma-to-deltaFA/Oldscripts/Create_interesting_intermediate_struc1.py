...
depth = 1
b_vec =  np.array([np.sqrt(2)*cspbi3.l, 2*cspbi3.l, np.sqrt(2)*cspbi3.l])
...
for k in range(depth):
    for j in range(height):
        ...
        for z_shift, face in zip(shifts.get((j, k), shifts['default']), faces.get((j, k), faces['default'])):
            trans, rot = cspbi3.add_octahedra(trans = trans, z_shift = z_shift, rot = rot*face)
...
write('interesting_struc.xyz', cspbi3.atoms, format="extxyz")
